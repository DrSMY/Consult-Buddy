...
Suggest additional lab tests, additional supplements, lab order notes, and clinical notes tailored to this patient and prescribed regimen.`;
    ...
    // ---- REVIEW LAB TESTS (filter candidate labs to those pertinent to chosen peptides) ----
    if (action === "review-lab-tests") {
      const {
        patient_name,
        intake_answers,
        chosen_peptides,
      ...
      if (!Array.isArray(candidate_lab_tests) || candidate_lab_tests.length === 0) {
        return new Response(JSON.stringify({ error: "candidate_lab_tests is required" }), {
          status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }

      const safeName = sanitizeString(patient_name, 100) || "Patient";
      const reviewSystem = `You are a clinical peptide therapy assistant. Given the peptides chosen for a patient and a candidate list of lab tests, return ONLY the tests that are clinically pertinent to safely monitor or evaluate response to THOSE specific peptides for THIS patient. Remove tests unrelated to the chosen regimen. Be conservative — if a test provides meaningful safety or efficacy value for the regimen or patient's history, keep it.`;

      const reviewUser = `## Patient
${safeName}
...
## Candidate Lab Tests
${JSON.stringify(candidate_lab_tests, null, 2)}

Classify each candidate as pertinent or not pertinent. Provide a short reason for the ones removed and a brief overall rationale.`;

      const rResp = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          tools: [{
            type: "function",
            function: {
              name: "review_lab_tests",
              description: "Return which candidate lab tests are pertinent to the chosen peptides.",
              parameters: {
                type: "object",
                properties: {
                  pertinent: { type: "array", items: { type: "string" } },
                  not_pertinent: {
                    type: "array",
                    items: {
                      type: "object",
                ...
                required: ["pertinent", "not_pertinent", "rationale"],
              },
            },
          }],
          tool_choice: { type: "function", function: { name: "review_lab_tests" } },
        }),
      });

      if (!rResp.ok) {
        if (rResp.status === 429) return new Response(JSON.stringify({ error: "Rate limit exceeded. Please try again shortly." }), { status: 429, headers: { ...corsHeaders, "Content-Type": "application/json" } });
        if (rResp.status === 402) return new Response(JSON.stringify({ error: "AI credits exhausted. Please add credits." }), { status: 402, headers: { ...corsHeaders, "Content-Type": "application/json" } });
        const t = await rResp.text();
        console.error("AI gateway error (review-lab-tests):", rResp.status, t);
        throw new Error("AI gateway error");
      }

      const rdata = await rResp.json();
      const rtool = rdata.choices?.[0]?.message?.tool_calls?.[0];
      if (!rtool) throw new Error("No tool call in review-lab-tests response");
      const rparsed = JSON.parse(rtool.function.arguments);
      rparsed.rationale = sanitizeGuideOutput(rparsed.rationale || "");
      return new Response(JSON.stringify(rparsed), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
